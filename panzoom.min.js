	...
	})()(
		{
			1: [
				function (require, module, exports) {
					...
					var domTextSelectionInterceptor =
						createTextSelectionInterceptor();
					var fakeTextSelectorInterceptor =
						createTextSelectionInterceptor(true);
					...
					function createPanZoom(domElement, options) {
						...
						if (!panController) {
							if (makeSvgController.canAttach(domElement)) {
								panController = makeSvgController(
									domElement,
									options
								);
							} else if (
								makeDomController.canAttach(domElement)
							) {
								panController = makeDomController(
									domElement,
									options
								);
							}
						...
						var filterKey =
							typeof options.filterKey === "function"
								...
								: noop;
						var pinchSpeed =
							typeof options.pinchSpeed === "number"
								? options.pinchSpeed
								: 1;
						...
						var speed =
							typeof options.zoomSpeed === "number"
								? options.zoomSpeed
								: defaultZoomSpeed;
						var transformOrigin = parseTransformOrigin(
						...
						var smoothScroll;
						if (
							"smoothScroll" in options &&
							!options.smoothScroll
						) {
							smoothScroll = rigidScroll();
						} else {
							smoothScroll = kinetic(
								getPoint,
								scroll,
								options.smoothScroll
							);
						...
						eventify(api);
						var initialX =
							typeof options.initialX === "number"
								? options.initialX
								: transform.x;
						var initialY =
						...
						function showRectangle(rect) {
							...
							transform.x =
								-(rect.left + rectWidth / 2) * scale +
								size.x / 2;
							transform.y =
								-(rect.top + rectHeight / 2) * scale +
								size.y / 2;
						...
						function autocenter() {
							...
							if (sceneBoundingBox) {
								...
								w =
									sceneBoundingBox.right -
									sceneBoundingBox.left;
								h =
									sceneBoundingBox.bottom -
									sceneBoundingBox.top;
							} else {
							...
							var scale = Math.min(dw, dh);
							transform.x =
								-(bbox.left + bbox.width / 2) * scale +
								w / 2 +
								left;
							transform.y =
								-(bbox.top + bbox.height / 2) * scale +
								h / 2 +
								top;
							transform.scale = scale;
						}
						...
						function setTransformOrigin(newTransformOrigin) {
							transformOrigin =
								parseTransformOrigin(newTransformOrigin);
						}
						...
						function getClientRect() {
							...
							return {
								...
								bottom:
									bbox.height * transform.scale + leftTop.y,
							};
						}
						function client(x, y) {
							return {
						...
						function zoomByRatio(clientX, clientY, ratio) {
							...
							transform.x =
								size.x - ratio * (size.x - transform.x);
							transform.y =
								size.y - ratio * (size.y - transform.y);
							if (
								bounds &&
								boundsPadding === 1 &&
								minZoom === 1
						...
						function onKeyDown(e) {
							...
							if (z) {
								var scaleMultiplier = getScaleMultiplier(
									z * 100
								);
						...
						function handleTouchMove(e) {
							if (e.touches.length === 1) {
								...
								var t1 = e.touches[0];
								var t2 = e.touches[1];
								var currentPinchLength = getPinchZoomLength(
									t1,
									t2
								);
								var scaleMultiplier =
									1 +
									(currentPinchLength / pinchZoomLength - 1) *
										pinchSpeed;
								...
								mouseX =
									...
									2;
								mouseY =
									(firstTouchPoint.y + secondTouchPoint.y) /
									2;
								if (transformOrigin) {
					...
					function autoRun() {
						...
						if (!query) return;
						var globalName =
							panzoomScript.getAttribute("name") || "pz";
						...
						function collectOptions(script) {
							...
							for (var i = 0; i < attrs.length; ++i) {
								var attr = attrs[i];
								var nameValue =
									getPanzoomAttributeNameValue(attr);
								if (nameValue) {
			...
			5: [
				function (require, module, exports) {
					...
					function isSVGElement(element) {
						return (
							element && element.ownerSVGElement && element.getCTM
						);
					}
				},
				{},
						...
						return {
							...
							fire: function (eventName) {
								...
								if (arguments.length > 1) {
									fireArguments = Array.prototype.splice.call(
										arguments,
										1
									);
								}
